...
def reset_order_DB():
    ...
    with open("FastAPI_DB/data/food_delivery.csv", newline="") as f:
        ...
        for row in reader:
            new_order = create_orders(OrderCreate(order_id=row[0], restaurant_id=int(row[1]), food_item=row[2], 
                                                    order_time=row[3], delivery_time=row[4], delivery_distance=float(row[5]), 
                                                    order_value=float(row[6]), delivery_method=row[7], traffic_condition=row[8], 
